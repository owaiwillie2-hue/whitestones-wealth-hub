          ...
          {investmentOptions.map((option) => (
            <Card key={option.id} className="shadow-soft hover:shadow-medium transition-smooth">
              ...
              <CardContent>
                ...
                <div className="mt-4">
                  {/* Route each card to its company info page when available */}
                  {option.id === 'cryptocurrencies' ? (
                    <Link to="/cryptocurrencies" className="text-primary hover:underline text-sm">Learn more about Bitcoin</Link>
                  ) : (
                    <Link
                      to={`/company/${option.id === 'oil-gas' ? 'oil-and-gas' : option.id === 'real-estate' ? 'real-estate' : option.id === 'loan' ? 'loan' : option.id === 'nft' ? 'nft' : option.id === 'retirement' ? 'retirement' : option.id === 'investments' ? 'investments' : option.id}`}
